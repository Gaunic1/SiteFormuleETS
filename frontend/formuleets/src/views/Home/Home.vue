<template>
  <div>
    <!-- PRESENTATION -->
    <div
      class="h-screen overflow-hidden bg-center bg-cover"
      lazy-background="/static/home/fond.svg"
    >
      <div
        class="flex flex-col landscape:flex-row tb:flex-row justify-center items-center min-h-screen"
      >
        <!-- TITLE -->
        <div
          class="flex-1 landscape:h-screen tb:h-screen w-full landscape:w-1/2 tb:w-1/2 flex flex-col justify-end landscape:justify-center tb:justify-center items-center z-20 text-dark-mode-2 p-10"
          data-aos="fade-right"
        >
          <img
            src="/static/home/logo-light.png"
            alt="logo"
            class="dark:block hidden w-full min-w-calc -m-2"
          />
          <img
            src="/static/home/logo-dark.png"
            alt="logo"
            class="dark:hidden flex w-full min-w-calc -m-2"
          />
          <p
            class="mt-7 dark:text-white text-center landscape:text-left tb:text-left"
          >
            {{ $t("message.home.presentation") }}
          </p>
        </div>

        <!-- ANIMATED FORMULE -->
        <div
          class="flex-1 landscape:h-screen tb:h-screen w-full landscape:w-1/3 tb:w-1/3 justify-center landscape:justify-end tb:justify-end items-start landscape:items-center tb:items-center flex z-0"
        >
          <img
            ref="formule"
    ...
    <!-- MAIN CONTENT -->
    <div
      class="flex-initial w-full z-10 bg-first overflow-x-hidden bg-white dark:bg-dark-mode"
    >
      ...
      <ParallaxHome
        :item="{
          text: 'message.home.aboutUs',
          image: '/static/home/fond-pc.png',
          mobileImage: '/static/home/fond-mobile.png',
        }"
        :isMobile="isMobile"
        position="left"
      ></ParallaxHome>
      ...
      <hr
        ...
        lazy-reset
        class="border-red-600 m-3 hidden nesthub:block"
      />
...
export default {
  ...
  mounted() {
    this.formule = this.$refs.formule;
    this.$data.mobileSize = "1024px";
    document.addEventListener("scroll", this.scrollFormule);
  },
  beforeUnmount() {
    document.removeEventListener("scroll", this.scrollFormule.bind(this));
  },
  methods: {
    scrollFormule() {
      ...
      if (scroll < window.innerHeight) {
        ...
        if (formule) {
          formule.style.transform = `translate(${-scroll}px, ${scroll}px) scale(1.25)`;
