    ...
    lazy-background="/static/home/fond.svg">
      ...
      <div class="flex-initial w-full z-10 bg-first overflow-x-hidden bg-white dark:bg-dark-mode">
        ...
        <template v-for="(item, index) of home" :key="item.text">
          ...
          <!-- BORDER -->
          <hr lazy-animation="zoomin" lazy-reset class="border-red-600 mr-3 ml-3">

          <!-- CONTENT GENERATION -->
          <div class="flex flex-col flex-wrap w-full min-h-screen 
          justify-center items-center bg-cover bg-center bg-fixed"
          :lazy-background="'/static/home/' + (index%2 == 0 ? 'fond2.svg' : 'fond.svg')">

            <div class="h-64 overflow-hidden flex justify-center items-center w-full relative bg-cover bg-center bg-fixed"
            :lazy-background="item.image">
              <!-- OVERLAY -->
              <div class="absolute top-0 left-0 w-full h-full opacity-50 bg-dark-mode z-0"></div>

              <h1 class="text-red-600 text-5xl m-3 text-center 
            ...
            class="flex-initial p-5 w-full text-center">
              <p class="mt-3 text-black dark:text-white text-lg" v-html="$t(item.text + '.text')"></p>
            </div>
